...
// IsLocked checks if clipboard is currently locked by another process
func (m *Manager) IsLocked() bool {
	if windowsClipboardReader, ok := m.reader.(*WindowsClipboardReader); ok {
		return windowsClipboardReader.isClipboardLocked()
	}
	return false
}
