...
func main() {
	...
	// 使用临时目录进行测试，避免影响实际配置
	tempConfigDir, err := os.MkdirTemp("", "to_icalendar_cache_test_*")
	if err != nil {
	...
	imageProcessor, err := processors.NewImageProcessorWithNormalizerAndDir(difyProcessor, nil, tempConfigDir)
	if err != nil {
